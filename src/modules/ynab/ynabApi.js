...
async function getAccounts(api, budgetId) {
  try {
    const transactionResponse = await api.accounts.getAccounts(budgetId);
...
function simplifyTransactions(transactions) {
  return transactions.map((t) => {
    ...
    const amountConverted = ynab.utils.convertMilliUnitsToCurrencyAmount(
      amount,
      2,
    );
    return {
...
async function getTransactionsByAccount(api, budgetId, accountId) {
  try {
    const transactionResponse = await api.transactions.getTransactionsByAccount(
      budgetId,
      accountId,
    );
...
async function getAndTransformTransactions(
  api,
  budgetId,
  accountId,
  startDate,
